...
class PlantImg extends StatefulWidget {
  const PlantImg({super.key});

  @override
  State<PlantImg> createState() => _PlantImgState();
}

class _PlantImgState extends State<PlantImg> {
  File? selectedImage;
  List imageList = [];
  String fileName = "";
  int? userID;
  ...
  Widget imageProfile() {
    return imageList.isNotEmpty
        ? CachedNetworkImage(
            imageUrl: "http://${host()}${imageList[0]['plant_img']}",
  ...
  Future getImage() async {
    var url = Uri.http(host(), '/media/plant/${userID}/');
  ...
  Future uploadImage() async {
    final request = http.MultipartRequest(
        'PATCH', Uri.parse("http://${host()}/petch/img-plant/${IDimg}/"));
